...
def extract_binary_mask(class_indices):
    ...
    """
    seg_nifti_img, _ = load_multilabel_nifti("code/temp_images/temp_ts/output_mask.nii")
...
def get_pulmonary_mask(ct):
    ...
    ct_path = "code/temp_images/temp_ct.nii.gz"
    sitk.WriteImage(ct_sitk, ct_path)

    output_mask = totalsegmentator(ct_path, task='total', output="code/temp_images/temp_ts_output")
    pulmonary_mask = sitk.ReadImage("code/temp_images/temp_ts_output/pulmonary_vein.nii.gz")
